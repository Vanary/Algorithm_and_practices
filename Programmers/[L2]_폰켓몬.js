...
function solution(numsArr) {
  const maxAmount = numsArr.length / 2;
  const result = new Set();

  numsArr.forEach(num => result.add(num)); // 포켓몬 목록을 Set에 넣어서 중복을 제거
  const numOfPokeMons = [...result].length; // 중복을 제거한 포켓몬 가짓수를 확인

  // 가짓수가 가져갈 수보다 크면 종류 갯수 = 가져갈 수 있는 포켓몬 수
  return numOfPokeMons >= maxAmount ? maxAmount : numOfPokeMons;
}
